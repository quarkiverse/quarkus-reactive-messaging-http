...
# The maximum size of a queue holding pending messages, i.e. messages waiting to receive an
# acknowledgment. `1` by default.
# Recommended to use with protocolVersion=HTTP_2 to reuse connection.
mp.messaging.outgoing.<channelName>.maxInflightMessages=1000

# Whether the client waits for the request completion before acknowledging the message. 'true' by default.
mp.messaging.outgoing.<channelName>.waitForCompletion=true

# HTTP protocol version. 'HTTP_1_1' by default.
mp.messaging.outgoing.<channelName>.protocolVersion=HTTP_2
...
# Whether tracing is enabled or disabled. 'true' by default.
mp.messaging.incoming.<channelName>.tracing-enabled=false
...
When the `quarkus-opentelemetry` extension is present, an incoming HTTP request's OpenTelemetry
`Context` (extracted from its `traceparent`/`tracestate` headers, same as any other HTTP endpoint) is
captured and attached to the `Message` so that the `@Incoming` consumer runs within the same trace as
the inbound request, instead of starting a disconnected trace. This is enabled by default and can be
turned off per channel with `tracing-enabled=false`:
...
Outgoing HTTP requests sent by the `HttpSink` don't need any extra configuration for trace propagation:
they go through the same Vert.x HTTP client that `quarkus-opentelemetry` already instruments, so a
`traceparent` header carrying the current trace is injected automatically whenever OpenTelemetry is
present.
